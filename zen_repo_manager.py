...
from find_all_local_repos import (
    Colors, print_colored, is_git_repository, get_git_info,
    scan_for_repositories, get_default_scan_paths
)
from clone_all_repos import (
    parse_arguments as parse_clone_args, get_configuration,
    get_github_token, ensure_destination_dir, confirm_action,
    fetch_all_repos, repo_exists_locally, clone_repository,
    update_repository, save_results_to_json
)
...
class ZenRepoManager:
    ...
    def _print_status_report(self, report: Dict) -> None:
        ...
        for branch, count in sorted(report['by_branch'].items(), key=lambda x: x[1], reverse=True)[:10]:
            print_colored(f"  {branch}: {count}", self.colors.CYAN)

        print_colored(f"\n⚠️  Issues detected:", self.colors.YELLOW)
        print_colored(f"  📝 Uncommitted changes: {report['uncommitted_changes']}", self.colors.YELLOW)
        ...
        if not repo.get('last_commit'):
            ...
            try:
                ...
                if days_since_commit > 90:
                    recommendations.append(f"Last commit was {days_since_commit} days ago - consider activity")
            except:
                pass
    ...
    def _print_audit_report(self, audit: Dict) -> None:
        ...
        print_colored(f"✅ Valid repositories: {summary['by_status'].get('valid', 0)}", self.colors.GREEN)

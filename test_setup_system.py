...
class SetupTester:
    ...
    def create_test_environment(self, name: str, scenario: str) -> Path:
        ...
        if scenario == "clean":
            ...
            subprocess.run(
                ["git", "config", "user.email", "test@example.com"], cwd=test_zenos, check=True
            )
    ...
    def run_setup_test(self, name: str, scenario: str, options: list = None) -> dict:
        ...
        try:
            result = subprocess.run(cmd, cwd=test_dir, capture_output=True, text=True, timeout=300)
            ...
            return {"name": name, "scenario": scenario, "success": False, "error": "Timeout"}
        except Exception as e:
            console.print(f"💥 {name} crashed: {e}")
            return {"name": name, "scenario": scenario, "success": False, "error": str(e)}
    ...
    def print_summary(self):
        ...
        for result in self.test_results:
            ...
            table.add_row(result["name"], result["scenario"], status, notes)
        ...
        if failed == 0:
            console.print(
                Panel.fit(
                    "🎉 All tests passed! Setup system is working perfectly!", style="bold green"
                )
            )
        else:
            console.print(
                Panel.fit(
                    f"⚠️  {failed} test(s) failed. Check the details above.", style="bold yellow"
                )
            )
...
@click.option(
    "--scenario",
    type=click.Choice(["all", "clean", "dirty", "partial", "phases"]),
    default="all",

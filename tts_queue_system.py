    ...
    def can_process(self) -> bool:
        ...
        with self.lock:
            ...
            if len(self.requests) >= self.config.rate_limit_per_minute:
                self.logger.warning(
                    f"Rate limit exceeded: {len(self.requests)} requests in last minute"
                )
                return False
...
class TTSWorker:
    ...
    async def process_message(self, message: TTSMessage, tts_engine: Callable) -> bool:
        ...
        try:
            ...
            if not self.audio_manager.can_play_audio(estimated_duration):
                self.logger.info(
                    f"Audio overlap detected, waiting for slot for message {message.id}"
                )
                # Wait for audio slot
    ...
    async def _generate_audio(self, message: TTSMessage, tts_engine: Callable) -> bytes:
        ...
        if asyncio.iscoroutinefunction(tts_engine):
            ...
            # Run blocking function in executor if needed, but here we assume it's fast or handles its own blocking
            # For better async hygiene, blocking calls should be offloaded, but keeping it simple for now
...
class TTSQueueManager:
    ...
    def __init__(self, config: TTSConfig = None):
        ...
        # Statistics
        self.stats = {"total_processed": 0, "total_failed": 0, "queue_size": 0, "active_workers": 0}
    ...
    def add_message(
        ...
        message = TTSMessage(
            text=text, priority=priority, metadata=metadata or {}, created_by=created_by
        )
...
async def example_usage():
    ...
    try:
        ...
        donation_messages = [
            ...
            (
                "New subscriber: Sarah! Welcome to the community!",

...
Environment Variables:
  ...
  REPO_DEST_DIR    - Default destination directory
    ...
    parser.add_argument(
        "-d", "--destination", type=Path, help="Destination directory for repositories"
    )
...
def fetch_all_repos(
    token: str, username: str, include_private: bool = False, exclude_forks: bool = False
) -> List[Dict]:
...
def update_repository(repo_name: str, destination: Path, dry_run: bool = False) -> Tuple[bool, str]:
    ...
    try:
        ...
        result = subprocess.run(
            ["git", "branch", "--show-current"], capture_output=True, text=True, cwd=repo_path
        )
        ...
        # Pull latest changes
        result = subprocess.run(["git", "pull"], capture_output=True, text=True, cwd=repo_path)
...
def main():
    ...
    print_colored(
        ...
        Colors.YELLOW if config["dry_run"] else Colors.GREEN,
    )
    ...
    for username in config["usernames"]:
        ...
        repos = fetch_all_repos(token, username, config["include_private"], config["exclude_forks"])
        ...
        for repo in repos:
            ...
            print_colored(
                f"\n{visibility} {repo_name}{fork_indicator} ({repo.get('language', 'Unknown')})",
                Colors.WHITE,
            )
    ...
    if total_stats["failed"] > 0:
        print_colored(
            f"\n⚠️  {total_stats['failed']} operations failed. Check the output above for details.",
            Colors.YELLOW,
        )
        sys.exit(1)
    elif config["dry_run"]:

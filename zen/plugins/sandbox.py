...
class PluginSandbox:
    ...
    async def execute_in_sandbox(self, sandbox_id: str, command: List[str], 
                                input_data: Any = None, timeout: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            process = await self._start_limited_process(
                command, sandbox_path, sandbox_id
            )
    ...
    async def _start_limited_process(self, command: List[str], 
                                   sandbox_path: Path, sandbox_id: str) -> subprocess.Popen:

...
class PluginExecutor:
    ...
    async def execute_plugin(self, plugin_id: str, procedure_id: str, 
                           input_data: Any, context: ExecutionContext) -> ExecutionResult:
        ...
        try:
            ...
            result = await self._execute_procedure(
                plugin_instance, procedure, input_data, context
            )
    ...
    async def execute_plugin_chain(self, plugin_chain: List[Dict[str, Any]], 
                                 input_data: Any, context: ExecutionContext) -> ExecutionResult:
    ...
    async def _execute_procedure(self, plugin_instance: Any, procedure: Dict[str, Any], 
                               input_data: Any, context: ExecutionContext) -> ExecutionResult:
        ...
        try:
            ...
            return ExecutionResult(
                success=False,
                data=None,
                error=f"Procedure execution error: {str(e)}"
            )
        ...
        try:
            ...
            if not procedures:
                return ExecutionResult(
                    success=False,
                    data=None,
                    error="No procedures available for testing"
                )
            ...
            return ExecutionResult(
                success=False,
                data=None,
                error=f"Plugin test error: {str(e)}"
            )
...
async def execute_plugin(plugin_id: str, procedure_id: str, 
                        input_data: Any, context: ExecutionContext) -> ExecutionResult:

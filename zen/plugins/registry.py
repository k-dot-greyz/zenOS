...
class PluginRegistry:
    ...
    def search_plugins(self, query: str) -> List[PluginEntry]:
        ...
        for entry in self.plugins.values():
            if (query_lower in entry.manifest.name.lower() or
                query_lower in entry.manifest.description.lower() or
                any(query_lower in cap.lower() for cap in entry.manifest.capabilities)):

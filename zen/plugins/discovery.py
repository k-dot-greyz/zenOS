...
class PluginDiscovery:
    ...
    async def search_plugins(self, query: str, category: Optional[str] = None, 
                           limit: int = 20) -> List[DiscoveredPlugin]:
...
async def search_plugins(query: str, category: Optional[str] = None, 
                        limit: int = 20, github_token: Optional[str] = None) -> List[DiscoveredPlugin]:
    """Search for plugins on GitHub"""
    async with PluginDiscovery(github_token) as discovery:
        return await discovery.search_plugins(query, category, limit)

async def discover_trending(limit: int = 20, github_token: Optional[str] = None) -> List[DiscoveredPlugin]:
    """Discover trending plugins"""
    async with PluginDiscovery(github_token) as discovery:
        return await discovery.discover_trending(limit)

async def discover_featured(limit: int = 10, github_token: Optional[str] = None) -> List[DiscoveredPlugin]:

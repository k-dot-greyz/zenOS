...
class ConversationProcessor:
    ...
    async def _generate_summary(self, conversation: Conversation) -> str:
        ...
        assistant_messages = [
            msg.content for msg in conversation.messages if msg.role == MessageRole.ASSISTANT
        ]
    ...
    async def _generate_tags(self, conversation: Conversation) -> List[str]:
        ...
        if any(
            word in all_text
            for word in ["code", "programming", "function", "variable", "algorithm"]
        ):
        ...
        if any(
            word in all_text for word in ["ai", "machine learning", "neural", "model", "training"]
        ):
            tags.append("ai")
        ...
        for i, message in enumerate(conversation.messages):
            if message.role == MessageRole.ASSISTANT:
                ...
                if any(
                    ...
                    for word in ["definition", "means", "refers to", "is a", "are"]
                ):
                    entry = KnowledgeEntry(

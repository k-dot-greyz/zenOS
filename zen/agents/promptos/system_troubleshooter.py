...
class SystemTroubleshooterAgent(Agent):
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.voice = "Methodical, diagnostic-focused, and solution-oriented with step-by-step guidance"
    ...
    def diagnose_and_fix(self, query: str, context: Optional[Dict] = None) -> str:
        ...
        report = self._generate_troubleshooting_report(diagnostic_result, analysis, fixes, applied_fixes)
    ...
    def _check_git_status(self) -> Dict:
        ...
        try:
            ...
            # Check git config
            user_name = self.safe_executor.run_command(["git", "config", "--global", "user.name"], timeout=5)
            user_email = self.safe_executor.run_command(["git", "config", "--global", "user.email"], timeout=5)
    ...
    def _generate_troubleshooting_report(self, diagnostic_result: Dict, analysis: Dict, 
                                       fixes: List[Dict], applied_fixes: List[Dict]) -> str:

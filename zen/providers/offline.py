...
class OfflineManager:
    ...
    def select_best_model(self, task: str = "chat") -> Optional[LocalModel]:
        ...
        suitable = [
            ...
            if (
                model.ram_required_mb < available_ram * 0.7  # Leave 30% free
                and task in model.capabilities
            )
        ]
    ...
    async def generate(self, prompt: str, model: Optional[str] = None, **kwargs) -> str:
        ...
        if model is None:
            ...
            # Find model config
            selected = next((m for m in self.available_models.values() if m.name == model), None)
            if not selected:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommended_model": (
                self.select_best_model("chat").name if self.select_best_model("chat") else None
            ),
        }

...
if command -v n8n &> /dev/null; then
    ...
    echo ""
    echo "   # Or using Docker (persistent volume + allow js-yaml in Code nodes):"
    echo "   docker run -it --name n8n \\"
    echo "     -p 5678:5678 \\"
    echo "     -e NODE_FUNCTION_ALLOW_EXTERNAL=js-yaml \\"
...
echo ""
echo "6. 📦 Enable js-yaml for Code nodes (REQUIRED for YAML parsing):"
echo "   - Set NODE_FUNCTION_ALLOW_EXTERNAL=js-yaml on the n8n process"
echo "     (and on Task Runners if you use them)"
echo "   - Install js-yaml where n8n can require it (n8n does not install it for you):"
echo "     # Docker: exec into the container, then:"
echo "     mkdir -p /home/node/.n8n && cd /home/node/.n8n"
echo "     npm init -y && npm install js-yaml"
echo "     # Example docker run (persistent volume + allowlist):"
echo "     docker run -it --name n8n \\"
echo "       -p 5678:5678 \\"
echo "       -e NODE_FUNCTION_ALLOW_EXTERNAL=js-yaml \\"
echo "       -v n8n_data:/home/node/.n8n \\"
echo "       n8nio/n8n"
echo "   - For npm / local installs:"
echo "     mkdir -p ~/.n8n && cd ~/.n8n"
echo "     npm init -y && npm install js-yaml"
echo "     export NODE_FUNCTION_ALLOW_EXTERNAL=js-yaml"
echo "   - Restart n8n after configuration (env must be set at process start)"
echo ""

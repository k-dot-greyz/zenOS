...
try:
    from zen.utils.dex_constants import PATTERNS, YAML_FRONT_RE
except ImportError:
    ...
    PATTERNS = {
        "dex_id": re.compile(
            r"^dex_id:\s*[\"']?(0x[0-9A-Fa-f]{2}:0x[0-9A-Fa-f]{2})[\"']?", re.MULTILINE
        ),
        "dex_type": re.compile(r"^dex_type:\s*[\"']?(\w+)[\"']?", re.MULTILINE),
        "status": re.compile(r"^status:\s*[\"']?(\w+)[\"']?", re.MULTILINE),
        "pe_id": re.compile(
            r"property_exchange_id:\s*[\"']?([a-zA-Z0-9:\-\.]+)[\"']?", re.MULTILINE
        ),
    }

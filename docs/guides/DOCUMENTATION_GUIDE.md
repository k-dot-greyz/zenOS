...
| :--- | :--- | :--- | :--- |
| **Active** | `docs/`, `README.md`, `dex/` | Current setup, architecture, and product behavior | Everyone |
...
│       └── CONVERSATION_ARCHIVE.md  # Legacy conversation log
├── dex/                         # Model and procedure catalog + protocol index
├── repos/
...
| model, procedure, dex, pokedex | `dex/`, `zen/dex/catalog.py` |
...
4. Consult `repos/registry.yaml` for which sibling repos exist; clone private repos only when credentials are available. Do not commit tokens, credentials, or secret endpoints. Non-secret GitHub repo URLs — including private repos marked `visibility: private` — belong in the switchboard.
